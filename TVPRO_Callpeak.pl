...
# perl TVPRO_Callpeak.pl Merged_p.bedGraph Merged_m.bedGraph
...
for (my $file = 0; $file < 2; $file++) {
	...
	while (defined (my $line = <FILE>) ) {
		...
		my $min1_check;
		if ($parameters[3]>0 and $parameters[4]>0 and $parameters[5]>0 and $parameters[6]>0) {
			$min1_check=1;
		}else{
			$min1_check=0;
		}
		if ($chr eq $last_chr) {
			...
			push @Temp_lines,"$parameters[3] $parameters[4] $parameters[5] $parameters[6] $parameters[7] $parameters[8] $parameters[9] $parameters[10]";
			...
			push @Min1,$min1_check;
			push @Temp_lines,"$parameters[3] $parameters[4] $parameters[5] $parameters[6]";
			push @Temp_reads,$reads;
			push @Temp_pos,$pos;
			shift @Min1;
			shift @Temp_lines;
